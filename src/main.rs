...
async fn main() {
    ...
    let texture: Texture2D = load_texture("assets/images/cube.png").await.unwrap();
    ...
    loop {
        ...
        for i in 0..xax {
            for j in (0..yax).rev() {
                let x = (cubes[(i * yax + j) as usize].x as f32 * 0.5 * 32.0
                    + cubes[(i * yax + j) as usize].y as f32 * 0.5 * 32.0)
                    + 0.2 * screen_width();
                let y = (cubes[(i * yax + j) as usize].x as f32 * 0.25 * 32.0
                    + cubes[(i * yax + j) as usize].y as f32 * -0.25 * 32.0)
                    + 0.5 * screen_height();
                // let y = j as f32 * 32.0;
